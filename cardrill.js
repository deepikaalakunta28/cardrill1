...
function cardealer(data){
    for(let user in data){
        let userid=data[user].id
        let year=data[user].car_year
        let make=data[user].car_make
        let mode=data[user].car_model
        if(userid===33){
            return `Car ${userid} is a ${year} ${make} ${mode}`
        }
    }
}
console.log(cardealer(inventory))
...
function carmodelsalphab(data){
    let result=[]
    for(let user in data){
        let carmodel=data[user].car_model
        result.push(carmodel)
    }
    return result.sort((a,b)=>a.localeCompare(b))
}
console.log(carmodelsalphab(inventory))
...
function getCarYears(data) {
  ...
  for (let user in data) {
    let caryear = data[user].car_year;
    result.push(caryear);
  }
...
function audibmw(data) {
  let result = [];
  for (let user in data) {
    let carmake = data[user].car_make;
...
console.log(JSON.stringify(audibmw(inventory)));
